...
class RetailVC: ViewController , TwicketSegmentedControlDelegate {

    var AreaId : String?
    var Details:PlaceDetails?
    var EnumRetail:RetailType?
    ...
    override func viewDidLoad() {
        ...
        SetUp()
        SetDataRetail()
        EnumRetail = "lang".localizable == "ar" ? .Offers : .About
    }
    ...
    ViewTop.addSubview(RetailImage)
    RetailImage.topAnchor.constraint(equalTo: ViewTop.topAnchor).isActive = true
    RetailImage.leadingAnchor.constraint(equalTo: ViewTop.leadingAnchor).isActive = true
    RetailImage.trailingAnchor.constraint(equalTo: ViewTop.trailingAnchor).isActive = true
    RetailImage.bottomAnchor.constraint(equalTo: ViewTop.bottomAnchor , constant: ControlHeight(-110)).isActive = true
                
    ViewTop.addSubview(RetailName)
    RetailName.topAnchor.constraint(equalTo: RetailImage.bottomAnchor,constant: ControlX(15)).isActive = true
    RetailName.leadingAnchor.constraint(equalTo: ViewTop.leadingAnchor,constant: ControlX(15)).isActive = true
    RetailName.widthAnchor.constraint(equalToConstant: ControlWidth(160)).isActive = true
    RetailName.heightAnchor.constraint(equalToConstant: ControlHeight(28)).isActive = true
        
    ViewTop.addSubview(ViewRating)
    ViewRating.layer.cornerRadius = ControlHeight(14)
    ViewRating.topAnchor.constraint(equalTo: RetailName.topAnchor).isActive = true
    ViewRating.bottomAnchor.constraint(equalTo: RetailName.bottomAnchor).isActive = true
    ViewRating.leadingAnchor.constraint(equalTo: RetailName.trailingAnchor,constant: ControlX(5)).isActive = true
    ViewRating.widthAnchor.constraint(equalToConstant: ControlWidth(60)).isActive = true
        
    ViewTop.addSubview(SavedButton)
    SavedButton.bottomAnchor.constraint(equalTo: RetailImage.bottomAnchor,constant: ControlY(20)).isActive = true
    ...
    SegmentedControl.topAnchor.constraint(equalTo: RetailName.bottomAnchor,constant: ControlX(10)).isActive = true
    ...
    view.addSubview(RetailCollection)
    RetailCollection.topAnchor.constraint(equalTo: ViewTop.bottomAnchor).isActive = true
    RetailCollection.leadingAnchor.constraint(equalTo: view.leadingAnchor,constant: ControlX(15)).isActive = true
    RetailCollection.trailingAnchor.constraint(equalTo: view.trailingAnchor,constant: ControlX(-15)).isActive = true
    RetailCollection.bottomAnchor.constraint(equalTo: view.bottomAnchor).isActive = true
    ...
    func setScrollPosition(_ position: CGFloat) {
        self.RetailCollection.contentOffset = CGPoint(x: self.RetailCollection.contentOffset.x, y: position)
    }
    ...
    func updateHeader() {
        ...
        self.RetailName.alpha = percentage
        self.RetailImage.alpha = percentage
    ...
    lazy var SegmentedControl:TwicketSegmentedControl = {
        ...
        View.setSegmentItems(["About".localizable, "Offers".localizable])
        View.backgroundColor = .clear
        View.delegate = self
        View.translatesAutoresizingMaskIntoConstraints = false
    ...
    if segmentIndex == "About".localizable && self.EnumRetail != .About {
    self.EnumRetail = .About
    }else if segmentIndex == "Offers".localizable && self.EnumRetail != .Offers {
    self.EnumRetail = .Offers
    }else{
    return
    }
        
    UIView.animate(withDuration: 0.3, delay: 0, options: []) {
    self.RetailCollection.alpha = 0
    } completion: { (_) in
    self.RetailCollection.alpha = 1
    self.RetailCollection.SetAnimations()
    }
    }

    var AboutId = "AboutId"
    var OffersId = "OffersId"
    lazy var RetailCollection: CollectionAnimations = {
        ...
        vc.register(RetailAboutCell.self, forCellWithReuseIdentifier: AboutId)
        vc.register(RetailOffersCell.self, forCellWithReuseIdentifier: OffersId)
        return vc
    }()
    ...
    self.RetailCollection.isHidden = Details != nil ? false : true
    self.RetailCollection.SetAnimations()
    self.ViewDots.endRefreshing {}
    self.ViewNoData.RefreshButton.addTarget(self, action: #selector(SetDataRetail), for: .touchUpInside)
    }

}

extension RetailVC : UICollectionViewDelegate , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout {
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if EnumRetail == .About {
        return 1
        }else{
        return Details?.Gallery.count ?? 0
        }
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if EnumRetail == .About {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: AboutId, for: indexPath) as! RetailAboutCell
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: OffersId, for: indexPath) as! RetailOffersCell
        cell.RetailImage.sd_setImage(with: URL(string: Details?.Gallery[indexPath.item].mediaPath ?? ""), placeholderImage: UIImage(named: "Group 26056"))
        return cell
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        return CGSize(width: collectionView.frame.width , height: estimatedFrame + ControlWidth(20))
        }else{
        return CGSize(width: collectionView.frame.width , height: ControlWidth(230))
        }
    }
...
//    } DictionaryData: { data in
        ...
        let data = ["id":"","screenId":1,"screenName":"","isFavorite":true,"coverPhotoPath":"https://onepagelove.imgix.net/2022/06/opl-screenshot-5.jpg?w=540&max-h=540&fit=crop&fp-y=0&auto=compress","title":"Test String","rate":3.5
                     ...
                     "gallery":[["mediaType":"","mediaPath":"https://img.freepik.com/free-vector/special-offer-creative-sale-banner-design_1017-16284.jpg?1"],
                                ["mediaType":"","mediaPath":"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTrM_t_ezRXUNMXkIATHoqYgkruw5CZD__ViL61wBas2a--cRjdWscMAUG3oNnTz_QajU0&usqp=CAU"],
                                ["mediaType":"","mediaPath":"https://as1.ftcdn.net/v2/jpg/02/91/61/20/1000_F_291612093_meNenTib4Fl1p5EiveFd0mt6RA0iqU4T.jpg"],
                                ["mediaType":"","mediaPath":"https://www.shutterstock.com/image-vector/special-offer-paper-white-background-260nw-448282759.jpg"],
                                ["mediaType":"","mediaPath":"https://as1.ftcdn.net/v2/jpg/02/91/61/20/1000_F_291612093_meNenTib4Fl1p5EiveFd0mt6RA0iqU4T.jpg"],
                                ["mediaType":"","mediaPath":"https://img.freepik.com/free-vector/special-offer-creative-sale-banner-design_1017-16284.jpg?1"],
        ...
        DispatchQueue.main.asyncAfter(wallDeadline: .now() + 2) {
            ...
            self.Dismiss.TextLabel = self.Details?.title ?? "Retail"
            self.RetailName.text = self.Details?.title ?? "Retail"
            self.RetailImage.sd_setImage(with: URL(string: self.Details?.coverPhotoPath ?? ""), placeholderImage: UIImage(named: "Group 26056"))
            ...
            self.RetailCollection.reloadData()
            self.IfNoData()
        }
//    } ArrayOfDictionary: { _ in
//    } Err: { error in
//    self.IfNoData()
//    self.SetUpIsError(error, true) {self.SetDataRetail()}
//    }
    }
