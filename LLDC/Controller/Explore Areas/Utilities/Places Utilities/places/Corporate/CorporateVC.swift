...
class CorporateVC : ViewController , TwicketSegmentedControlDelegate {
    ...
    var EnumCorporate:CorporateType?
    ...
    override func viewDidLoad() {
        ...
        SetUp()
        SetDataCorporate()
        EnumCorporate = "lang".localizable == "ar" ? .Gallery : .About
    }
    ...
    ViewTop.addSubview(CorporateImage)
    CorporateImage.topAnchor.constraint(equalTo: ViewTop.topAnchor).isActive = true
    CorporateImage.leadingAnchor.constraint(equalTo: ViewTop.leadingAnchor).isActive = true
    CorporateImage.trailingAnchor.constraint(equalTo: ViewTop.trailingAnchor).isActive = true
    CorporateImage.bottomAnchor.constraint(equalTo: ViewTop.bottomAnchor , constant: ControlHeight(-110)).isActive = true
    
    ViewTop.addSubview(CorporateName)
    CorporateName.topAnchor.constraint(equalTo: CorporateImage.bottomAnchor,constant: ControlX(15)).isActive = true
    CorporateName.leadingAnchor.constraint(equalTo: ViewTop.leadingAnchor,constant: ControlX(15)).isActive = true
    CorporateName.widthAnchor.constraint(equalToConstant: ControlWidth(160)).isActive = true
    CorporateName.heightAnchor.constraint(equalToConstant: ControlHeight(28)).isActive = true
        
    ViewTop.addSubview(ViewRating)
    ViewRating.layer.cornerRadius = ControlHeight(14)
    ViewRating.topAnchor.constraint(equalTo: CorporateName.topAnchor).isActive = true
    ViewRating.bottomAnchor.constraint(equalTo: CorporateName.bottomAnchor).isActive = true
    ViewRating.leadingAnchor.constraint(equalTo: CorporateName.trailingAnchor,constant: ControlX(5)).isActive = true
    ViewRating.widthAnchor.constraint(equalToConstant: ControlWidth(60)).isActive = true
        
    ViewTop.addSubview(SavedButton)
    SavedButton.bottomAnchor.constraint(equalTo: CorporateImage.bottomAnchor,constant: ControlY(20)).isActive = true
    ...
    SegmentedControl.topAnchor.constraint(equalTo: CorporateName.bottomAnchor,constant: ControlX(10)).isActive = true
    ...
    view.addSubview(CorporateCollection)
    CorporateCollection.topAnchor.constraint(equalTo: ViewTop.bottomAnchor).isActive = true
    CorporateCollection.leadingAnchor.constraint(equalTo: view.leadingAnchor,constant: ControlX(15)).isActive = true
    CorporateCollection.trailingAnchor.constraint(equalTo: view.trailingAnchor,constant: ControlX(-15)).isActive = true
    CorporateCollection.bottomAnchor.constraint(equalTo: view.bottomAnchor).isActive = true
    ...
    func setScrollPosition(_ position: CGFloat) {
        self.CorporateCollection.contentOffset = CGPoint(x: self.CorporateCollection.contentOffset.x, y: position)
    }
    ...
    func updateHeader() {
        ...
        self.CorporateName.alpha = percentage
        self.CorporateImage.alpha = percentage
    ...
    if segmentIndex == "About".localizable && self.EnumCorporate != .About {
    self.EnumCorporate = .About
    }else if segmentIndex == "Gallery".localizable && self.EnumCorporate != .Gallery {
    self.EnumCorporate = .Gallery
    }else{
    return
    }
        
    UIView.animate(withDuration: 0.3, delay: 0, options: []) {
    self.CorporateCollection.alpha = 0
    } completion: { (_) in
    self.CorporateCollection.alpha = 1
    self.CorporateCollection.SetAnimations()
    }
    }

    var AboutId = "AboutId"
    var GalleryId = "GalleryId"
    lazy var CorporateCollection: CollectionAnimations = {
        ...
        vc.register(CorporateAboutCell.self, forCellWithReuseIdentifier: AboutId)
        vc.register(CorporateGalleryCell.self, forCellWithReuseIdentifier: GalleryId)
        return vc
    }()
    ...
    self.CorporateCollection.isHidden = Details != nil ? false : true
    self.CorporateCollection.SetAnimations()
    self.ViewDots.endRefreshing {}
    self.ViewNoData.RefreshButton.addTarget(self, action: #selector(SetDataCorporate), for: .touchUpInside)
    }


}

extension CorporateVC : UICollectionViewDelegate , UICollectionViewDataSource , UICollectionViewDelegateFlowLayout, CorporateAboutDelegate ,MediaBrowserDelegate {
    
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if EnumCorporate == .About {
        return 1
        }else{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if EnumCorporate == .About {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: AboutId, for: indexPath) as! CorporateAboutCell
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: GalleryId, for: indexPath) as! CorporateGalleryCell
        cell.CorporateImage.image = imageArray[indexPath.item]
        return cell
        }
    }
    
    func CreateRequest(Cell: CorporateAboutCell) {
...
//    } DictionaryData: { data in
        ...
        DispatchQueue.main.asyncAfter(wallDeadline: .now() + 2) {
            ...
            self.Dismiss.TextLabel = self.Details?.title ?? "Corporate"
            self.CorporateName.text = self.Details?.title ?? "Corporate"
            self.CorporateImage.sd_setImage(with: URL(string: self.Details?.coverPhotoPath ?? ""), placeholderImage: UIImage(named: "Group 26056"))
                ...
                for item in Gallery {
                    SDWebImageManager.shared.loadImage(with: URL(string: item.mediaPath ?? ""), progress: nil) { image, _, error, _, _, _ in
                        if let Image = image {
                            ...
                            self.CorporateCollection.reloadData()
                        }else{
...
//    } Err: { error in
//    self.IfNoData()
//    self.SetUpIsError(error, true) {self.SetDataCorporate()}
//    }
    }
    ...
    let Image = Media(image: image, caption: self.Details?.title ?? "Corporate")
    thumbs.append(Image)
    }
    return thumbs[index]
    }
